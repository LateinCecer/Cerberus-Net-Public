...
public interface Worker {
    ...
    float computationsPerSecond(long currentTime);

    default float getLoad(long currentTime) {
        return (float) Math.pow(computationsPerSecond(currentTime), getPriority().weight);
    }
    ...
    WorkerGroup[] getGroups();

    boolean hasActiveGroup();
    boolean isInGroup(WorkerGroup group);
}
